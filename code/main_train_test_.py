...
parser.add_argument('--dataset',    type=str,   default='gtsrb', help='dataset to use [gtsrb, gtsrb2TT100K, belga2flickr, belga2toplogo]')
...
result_path = 'results__' + args.dataset
if not os.path.exists(result_path):
  os.makedirs(result_path)
outimg_path =  "./img_log__" + args.dataset
...
net.load_state_dict(torch.load("/drive0-storage/VPE/code/gtsrb_testBest_net.pth"))
net.cuda()
...
# # Assuming you have the tensor of size [1, 3, 64, 64]
# tensor = torch.randn(1, 3, 64, 64)

# # Transpose the tensor to size [64, 64, 3]
# transposed_tensor = tensor.permute(0, 2, 3, 1)

# # Print the size of the transposed tensor
...
def test(e, best_acc):
  ...
  for i, (input, target, template) in enumerate(testloader):
    ...
    print(target.size(), '\n')
    # plt.imshow(input)
    #transposed_tensor = input.permute(0, 2, 3, 1)
    # squeezed_input = input.squeeze()
    # print(squeezed_input.size())
    #squeezed_input = squeezed_input.to('cpu')
    #print(plt.imshow(squeezed_input))
    #utils.save_image(squeezed_input, str(c) + 'image.jpg')
    # c = c + 1
    # if c == 10:
    break
